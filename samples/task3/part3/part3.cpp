...
void matrix_mul_cl(const matrix<int>& a, const matrix<int>& b, matrix<int>& c, cl_device_id device)
{
    ...
    region[0] = m;
    region[1] = n;
    CL_CHECK_AND_RET(clEnqueueWriteImage(main_queue, a_buf, CL_FALSE, origin, region, 0, 0, const_cast<int*>(a.data()), 0, nullptr, &buf_e[0]));
    region[0] = l;
    region[1] = m;
    CL_CHECK_AND_RET(clEnqueueWriteImage(main_queue, b_buf, CL_FALSE, origin, region, 0, 0, const_cast<int*>(b.data()), 0, nullptr, &buf_e[1]));
    region[0] = l;
    region[1] = n;
    CL_CHECK_AND_RET(clEnqueueWriteImage(main_queue, c_buf, CL_FALSE, origin, region, 0, 0, c.data(), 0, nullptr, &buf_e[2]));
    ...
    CL_CHECK_AND_RET(clEnqueueReadImage(main_queue, c_buf, CL_TRUE, origin, region, 0, 0, c.data(), 1, &main_e, nullptr));
